...
describe('BaseEvent', () => {
  it('sets all fields on valid construction', () => {
    const evt = new TestEvent('tenant-uuid', 'CreateTenantCommand', 'corr-uuid');
  ...
  it('throws when tenant_id is missing', () => {
    expect(() => new TestEvent('', 'CreateTenantCommand', 'corr-uuid')).toThrow('tenant_id is required');
  });

  it('throws when source_command is missing', () => {
    expect(() => new TestEvent('tenant-uuid', '', 'corr-uuid')).toThrow('source_command is required');
  });

  // AC5
  it('throws when correlation_id is missing', () => {
    expect(() => new TestEvent('tenant-uuid', 'CreateTenantCommand', '')).toThrow('correlation_id is required');
  });
});
